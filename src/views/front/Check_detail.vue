          ...
          <div>
            <div>
              ...
              <button type="button" class="btn btn-main mb-2 rounded-0
                            " v-else data-toggle="collapse" data-target="#collapseExample" aria-expanded="false"
                aria-controls="collapseExample">去付款</button>
                  ...
                  <validation-observer v-slot="{ invalid }">
                    <form>
                      ...
                      <validation-provider rules="required|numeric|min:16" v-slot="{ errors, classes }">
                        <input type="tel" class="form-control mb-2" :class="classes" placeholder="信用卡號" name="信用卡號"
                          v-model="form.number" maxlength="16">
                        <span class="invalid-feedback mb-2">{{ errors[0] }}</span>
                      </validation-provider>

                      <validation-provider rules="required" v-slot="{ errors, classes }">
                        <input type="text" class="form-control mb-2" :class="classes" placeholder="持卡人姓名" name="持卡人姓名"
                          v-model="form.name">
                        <span class="invalid-feedback mb-2">{{ errors[0] }}</span>
                      </validation-provider>
                      <div class="form-row mb-2">
                        <div class="col">
                          <validation-provider rules="required|numeric|min:4" v-slot="{ errors, classes }">
                            <input type="tel" class="form-control mb-2" :class="classes" placeholder="到期日( MM / YY )"
                              name="到期日" v-model="form.dueDate" maxlength="4">
                            <span class="invalid-feedback mb-2">{{ errors[0] }}</span>
                          </validation-provider>
                        </div>
                        <div class="col">
                      ...
                      <button type="button" class="btn btn-main w-100 rounded-0" @click.prevent="checkout"
                        :disabled="invalid">結帳</button>
                    </form>
                  </validation-observer>
                </div>
              </div>
