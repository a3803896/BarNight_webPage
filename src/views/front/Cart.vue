<template>
  <div class="cart pb-3 overflow-hidden">
    <div class="container" v-if="cartProducts.length !== 0">
      <div class="d-flex justify-content-end">
        <button type="button" class="btn btn-outline-sub noto bold btn-sm rounded-0 mb-3"
          @click.prevent="clearAll">清空購物車</button>
      </div>
      <div
        class="row justify-content-center align-items-center align-items-lg-stretch text-white pt-lg-4 pb-lg-4 flex-column-reverse flex-lg-row mb-3">
        <div class="col-12 col-md-8 col-lg-4">
          <form class="py-3 py-lg-0 sticky-top">
            <validation-observer v-slot="{ invalid }">
              ...
              <validation-provider rules="required" v-slot="{ errors, classes }">
                ...
                <select v-model="form.payment" name="付款方式" id="payment" class="form-control mb-2" :class="classes">
              ...
              <button type="submit" :disabled="invalid" class="btn btn-main rounded-0" @click.prevent="sendOrder">
                送出訂單
              </button>
            </validation-observer>
          </form>
        ...
        <div class="col-12 col-md-8 col-lg-8">
          <div class="cartCard d-flex mb-3" v-for="item in cartProducts" :key="item.product.id">
            <img :src="item.product.imageUrl[0]" :alt="item.product.title" class="w-25">
          ...
          <h2 class="noto text-right text-main bold">Total：NT$<span v-text="totalPrice"></span></h2>
        </div>
      </div>
      <Mypagination :inner-pagination="pagination" @change-page="getCart"></Mypagination>
    </div>
    <div class="pt-3 pt-lg-5" v-else>
      <h2 class="text-white text-center mb-4">購物車是空的<br>馬上去點些<span class="text-main">美酒</span>吧！</h2>
      <div class="white-texture py-3 py-lg-5">
        <div class="container">
          <div class="row">
            <div class="col-12">
    ...
    editCart (which, item) {
      ...
      switch (which) {
        case 'plus':
          ...
          this.axios.patch(url, productPlus)
            .then((res) => {
              this.getCart()
              this.isLoading = false
            })
        ...
        case 'minus':
          if (item.quantity !== 1) {
            ...
            this.axios.patch(url, productMinus)
              .then((res) => {
                this.getCart()
                this.isLoading = false
              })
